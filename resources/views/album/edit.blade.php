...
@section('content')
    ...
    <form action="{{ route('edit.album', $album) }}" method="POST" class="album-container" enctype="multipart/form-data">
        ...
        <div class="rounded-lg w-80">
            ...
            <label class="text-sm font-bold uppercase text-gray-500" for="released_at">Release date:</label>
            <input class="input" type="date" name="released_at" id="released_at" value="{{ $album->released_at }}">
            <label class="text-sm font-bold uppercase text-gray-500" for="edition">Album edition:</label>
        ...
        <div class="album-container--info">
            ...
            <p class="m-3 text-sm font-bold uppercase text-gray-500">Album description.</p>
            ...
            <div>
                <p class="m-3 text-sm font-bold uppercase text-gray-500">Use markdown to write a review.</p>
                <textarea class="block border-t pt-3 text-lg input" name="review" id="review" cols="30" rows="10">{{ $album->review }}</textarea>
            </div>
        </div>
    </form>
